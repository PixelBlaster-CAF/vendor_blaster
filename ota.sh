...
VERSION="5.2"
DEVICE_NAME=$(awk -F'/PixelBlaster-CAF-5.2-|-G' '{print $2}' <<< "$BLASTER_TARGET_PACKAGE")
ZIP_NAME=$(awk -F$DEVICE_NAME/ '{print $2}' <<< "$BLASTER_TARGET_PACKAGE")
ID=$(md5sum $BLASTER_TARGET_PACKAGE | awk '{print $1}')
Date=$(grep "ro.build.date.utc=" out/target/product/$DEVICE_NAME/system/build.prop | cut -d "=" -f 2)
Size=$(wc -c $BLASTER_TARGET_PACKAGE | awk '{print $1}')
git clone https://github.com/PixelBlaster-releases/$DEVICE_NAME
cp $BLASTER_TARGET_PACKAGE $DEVICE_NAME/
cd $DEVICE_NAME
